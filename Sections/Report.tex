...
Because the default implementation in \Fref{lst:visitor-hand} line 12 uses the helper in line 27, \textit{VisitorName} does not need to implement anything for \textit{Window} to traverse into a \textit{Window}'s children.
...
\textit{AbstractFactoryAttribute} is defined in \Fref{lst:abstract-factory-attribute}.
...
\Fref{lst:trait-specifier} shows interpolation being implemented for the \textit{TraitSpecifier} -- specified in \Fref{lst:trait-specifier}.
