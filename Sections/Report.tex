...
\begin{enumerate}
	\item The abstract visitor, as defined in lines 3 to 14, maps to the UML for visitor, as defined in \Fref{sec:design-visitor}.
	...
	\item Making each element visitable in lines 40 to 58 which maps to the \textit{Visitable} UML, s defined in \Fref{sec:design-visitor}.
...
Because the default implementation in \Fref{lst:visitor-hand} line 12 uses the helper in line 27, \textit{NameVisitor} does not need to implement anything for \textit{Window} to traverse into a \textit{Window}'s children.
...
The test function in \Fref{lst:visitor-hand}, lines 50 to 55, show how to use this visitor.
...
The \textit{expand} method for the \textit{AbstractFactoryAttribute} model is defined as seen in \Fref{lst:abstract-factory-attribute}.
...
Line 11 parses the input context -- which is the \textit{AbstractGuiFactory} definition in the meta-code -- with line 12 parsing the macro inputs to \textit{AbstractFactoryAttribute} as defined in \Fref{lst:abstract-factory-attribute}.
Line 14 expands the inputs on the context as defined in \Fref{lst:abstract-factory-attribute}.

The \textit{interpolate\_traits} macro -- also being an attribute macro -- is shown in \Fref{lst:macro-patterns}.
...
\Fref{lst:trait-specifier} shows interpolation being implemented for the \textit{TraitSpecifier}.
...
Thus, line 9 in \Fref{lst:macro-patterns} will use the context input as a template to interpolate each \textit{TraitSpecifier} passed into the \textit{interpolate\_traits} macro.
...
The function-like -- as was defined in \Fref{sec:function-like-macro} -- \textit{visitor} macro is shown in \Fref{lst:macro-patterns}.
It uses the \textit{VisitorFunction} model defined in \Fref{lst:visitor-function} to parse the macro input in line 5.
