...
A metaprogram can, therefore, be refactored, abstracted, turned into library helpers, and tested \cite{lilis_15_01}.
...
The object code will be interleaved with the metaprogram, thus making it hard to distinguish between the two.
...
It allows writing the data as a ``string'' (enclosed in backtick quotes) in the syntactical form of the object language.
...
Let's discover the \textit{TokenStream}, \textit{syn}, and \textit{quote} by exploring the three flavors of \textit{Procedural Macros}.
...
The macro is used on line 19 with the derive attribute on an object.
