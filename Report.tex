...
Having ownership moving makes great memory guarantees within a function, but is annoying when calling another function as seen in \fref{lst:Rust/fn-move}. The \textit{take\_ownership} function takes ownership of the heap data which results in memory cleanup code correctly being inserted at the end of \textit{take\_ownership} scope on line 10. When \textit{main} calls \textit{take\_ownership}, \textit{a} becomes the new owner of \textit{s}'s data, thus making the call on line 5 invalid. When taking ownership like this is not desired the second form of ownership, borrowing, should be used instead.
