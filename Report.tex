...
Lines 9 to 12 shows the use of \textit{match} - called a \textit{switch} in most languages - to match against each possible enum variant.
Line 10 and 11 is each called a \textit{match arm}.
...
The reason being that matches need to be exhaustive - i.e. all variants need to be covered - in Rust.
...
Or using the \textit{if let} pattern as seen on line 14.
The \textit{if let} pattern also allows extraction of the enum object.
But since the object is not used inside the if block, no extraction needs to take place.
...
Thus, lines 16 and 18 are missing their ending semi-colon to return \textit{true} and \textit{false} from the \textit{if}.
The value returned from the \textit{if} is assigned to \textit{valid}.
Returning from a \textit{match} is quite common, especially with the special enum used for error handling.
...
The defition for \textit{Result} can be seen in \fref{lst:Rust/enum-result} on lines 1 to 4.
...
Else, in the event of an error, the \textit{Err} variant is returned with the error of type \textit{E} - like \textit{may\_error} on line 7.
