...
The scope start at the variable declaration and ends at the closing curly bracket for the code block the variable is in.
...
The initial variable is also invalidated at this point \cite{klabnik_2019_01}.
An analogy example would be to give a book to a friend.
The friend can do anything from anotating to burning the book as they feel fit.
...
But, because \textit{s} is a heap object, the compiler transfers ownership of the data from \textit{s} to \textit{t} and marks \textit{s} as invalid.

When trying to use the data on line 5, via \textit{s}, the compiler therefore throws an error saying that \textit{s} was moved.
In fact any reference to \textit{s} after line 3 will always give a compiler error.

Finally, the scope of \textit{t} ends on line 6.
Since the compiler is able to guarantee \textit{t} is the only variable owning the data, the compiler can free the memory on line 6.

\embed{Function taking ownership}{Rust/fn-move}

Having ownership moving makes great memory guarantees within a function, but is annoying when calling another function as seen in \fref{lst:Rust/fn-move}.
The \textit{take\_ownership} function takes ownership of the heap data which results in memory cleanup code correctly being inserted at the end of \textit{take\_ownership}'s scope on line 10.
When \textit{main} calls \textit{take\_ownership}, \textit{a} becomes the new owner of \textit{s}'s data, thus making the call on line 5 invalid.
When taking ownership like this is not desired the second form of ownership, borrowing, should be used instead.

\subsubsection{Borrowing}

Borrowing has a new variable take a references to data rather than becoming its new owner \cite{klabnik_2019_01}.
It is analogous to borrowing a book from a friend where a promise is made that the book will be returned to its owner once done with it.
...
The ampersand is also used in the call argument to signal that the called function will borrow the data.

But in Rust, all variables are immutable by default \cite{klabnik_2019_01}.
So trying to change the data in \textit{take\_borrow} causes an error stating the borrow is not mutable on line 9.
This is much the same as the borrowed book.
One would not make highlights and notes in a book that is borrowed, unless if explicit permission was given to do so by the owner.
...
Again, it is also used in the call to make it explicit that the function will modify the data.
Variables -- on the stack or heap -- also need to be declared \textit{mut} to be able to use them as mutable.
\cite{klabnik_2019_01}
...
\begin{itemize}
  ...
  \item Borrowed variables cannot be mutated.
        But more than one function can borrow the data at the same time in parallel and concurrent code.
...
% Box<Trait> is owned; &Trait is a reference.

% macro rules: Hygiene and token list

\subsection{Not quite OOP}
No single definition exists to qualify a language as being OOP.
Rust meets some of the definitions but not all of them.
The first definition Rust does meet is presented by the Gang of Four.

\subsubsection{Objects as Data and their Behaviour}
According to the Gang of Four, an OOP is made up of objects.
An object in turn holds both data and procedures that operate on the data \cite{gamma_94_01}.
Rust meets this requirement by holding the data in \textit{struct}s and having the operations defined in \textit{impl} blocks.

\paragraph{Struct}
A \textit{struct} is exactly the same as a \textit{struct} in C and other C like languages.
...
\embed[lastline=34]{Example of a Struct}{Rust/oop}

\paragraph{Methods}
The operations that can be performed on a struct is defined in an \textit{impl} block as seen in lines 7 - 19 in \fref{lst:Rust/oop}.
Notice how the ownership rules apply to this struct.
